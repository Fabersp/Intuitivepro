...
@interface TBV_Engenharia ()

@end

@implementation TBV_Engenharia
...
- (void)viewDidLoad {
    ...
    if ([vsCategoria isEqualToString: @"2"]) {
        vcCor = azul;
        self.title = @"ENGENHARIA";
    } else if ([vsCategoria isEqualToString: @"3"]) {
        vcCor = vermelho;
        self.title = @"JURÍDICO";
    } else if ([vsCategoria isEqualToString: @"4"]) {
        vcCor = amarelo;
        self.title = @"CONTÁBIL";
...
-(void)Loading {
    if (internetActive){
        ...
        NSString * urlStr = [NSString stringWithFormat:@"http://www.promastersolution.com.br/guia/api/lista_categoria.php?tipo_os=IOS&categ=%@&id_cidade=2301", vsCategoria];
...
- (IBAction)btnBuscar:(id)sender {
    ...
    clientes.vcCor = vcCor;
    clientes.vsCodCor = vsCodCor;
    clientes.vsIdCategoria = vsCategoria;
