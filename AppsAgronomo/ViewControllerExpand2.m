...
@interface ViewControllerExpand2 ()<UITableViewDataSource, UITableViewDelegate, IDMPhotoBrowserDelegate, SSPopupDelegate>
{
...
@implementation ViewControllerExpand2
...
@synthesize url_facebook, Url_Site, Url_instagram, Email, telefone1, telefone2, WhatAppConcat, telefone2WhatsApp, celular, Endereco;
...
//     @{NSForegroundColorAttributeName:Rgb2UIColor(255, 255, 255)}];
    ...
    if ([tipoProfiss  isEqual: @"1"]) {
        ...
        tituloHeader = [NSArray arrayWithObjects: @"Áreas de atuação", @"Endereço", @"Currículo", @"Site/Rede Social", nil];
    ...
    // dois
    [self Loading_dois:StrCurriculo];
    
    // rede social está dentro de pofissionais
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (section == 1) {
        ...
        Endereco = [[segundo objectAtIndex:indexPath.row] objectForKey:@"endereco"];
    ...
    if (section == 2) {
        cell.imgLocation.hidden = true;
        cell.lblName.text = [[terceiro objectAtIndex:indexPath.row] objectForKey:@"descricao"];
    ...
    if (section == 3) {
        ...
        if ([[quarto objectAtIndex:indexPath.row] containsString:@"@"]) {
        ...
        NSRange rangeAt = [[quarto objectAtIndex:indexPath.row] rangeOfString:@"@" options:NSCaseInsensitiveSearch];
        
        NSRange rangefacebook = [[quarto objectAtIndex:indexPath.row] rangeOfString:@"facebook" options:NSCaseInsensitiveSearch];
        
        NSRange rangeInstagram = [[quarto objectAtIndex:indexPath.row] rangeOfString:@"instagram" options:NSCaseInsensitiveSearch];
        ...
        if ([[quarto objectAtIndex:indexPath.row] containsString:@"facebook"]) {
        ...
        if ([[quarto objectAtIndex:indexPath.row] containsString:@"instagram"]) {
    ...
    if (section == 3) {
        ...
        if ([[quarto objectAtIndex:indexPath.row] containsString:@"@"]) {
            ...
            if ([MFMailComposeViewController canSendMail])
            {
                ...
                NSArray *toRecipients = [NSArray arrayWithObjects:[quarto objectAtIndex:indexPath.row], nil];
        ...
        if ([[quarto objectAtIndex:indexPath.row] containsString:@".com"] &&
            rangeAt.location == NSNotFound  &&
            rangefacebook.location == NSNotFound  &&
            rangeInstagram.location == NSNotFound ) {
            
            [[UIApplication sharedApplication] openURL:[NSURL URLWithString:[quarto objectAtIndex:indexPath.row]]];
        ...
        if ([[quarto objectAtIndex:indexPath.row] containsString:@"facebook"]) {
            NSString * UrlFacebook = [quarto objectAtIndex:indexPath.row];
...
//
        ...
        if ([[quarto objectAtIndex:indexPath.row] containsString:@"instagram"]) {
            ...
            NSString * UrlInstagram = [quarto objectAtIndex:indexPath.row];
            
            NSString * profileFacebook = [NSString stringWithFormat:@"instagram://user?username=%@", [[Profissional objectAtIndex:0] objectForKey:@"id_instagram"] ];
            
            NSURL *instagramURL = [NSURL URLWithString:profileFacebook];
...
-(void)Loading_dois:(NSString *) StrCurriculo {
...
-(void)Loading_Profissional {
    ...
    NSURLSessionTask *task = [[NSURLSession sharedSession] dataTaskWithURL:url completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [NSCharacterSet whitespaceCharacterSet]];
            telefone2WhatsApp = [[[Profissional objectAtIndex:0] objectForKey:@"telefoneWhatsApp"] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
            ...
            //imgGaleria.hidden = false;
            btnFavoritos.hidden = false;
            _lbNome.hidden = false;
            _lbConselho.hidden = false;
            _lbEspecializacao.hidden = false;
            _lbAtende.hidden = false;

            quarto = [[NSMutableArray alloc] init];

            if (![Email isEqual: @""]){
                [quarto addObject:Email];
            }
            if (![Url_Site isEqual: @""]){
                [quarto addObject:Url_Site];
            }
            if (![url_facebook isEqual: @""]){
                [quarto addObject:url_facebook];
            }
            if (![Url_instagram isEqual: @""]){
                [quarto addObject:Url_instagram];
            }
